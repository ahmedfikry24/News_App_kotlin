...
class CategoryAdapter(var items: List<Category>) : Adapter<CategoryAdapter.CategoryViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CategoryViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(
            if (viewType == 1) R.layout.left_item_category else R.layout.right_item_category,
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        cardView.setCardBackgroundColor(holder.itemView.context.getColor(item.color))
